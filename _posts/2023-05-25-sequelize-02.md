...
- **데이터 추가 (INSERT)**
  ...
  - **findOrCreate()**
    ...
    findOrCreate() 메소드는 없는 값이면 생성하고, 있는 값이면 값을 가져옵니다.

- **데이터 조회 (SELECT)**

  - **options**

    - **attributes** : 가져올 속성를 배열 형태로 지정
      ```js
      Model.findAll({
        attributes: ["user_id", "name"],
      });
      ```
    - **where** : 특정 레코드만 가져올 시 조건을 지정
      ```js
      Model.findAll({
        where: {
          age: 27,
  ...
  - **findAll(options)**
    ...
    // users 테이블 전체를 조회해서 그 결과값을 객체로 만들어 user변수에 넣어준다.
