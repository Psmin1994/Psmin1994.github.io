...
우리는 기본적인 사용자 **_id_**와 **_passport_**를 이용한 인증 방법인 **_local strategy(passport-local)_**를 사용해보겠습니다. (passport-local)
...
- 미들웨어 등록  
  passport.initialize : req 객체에 passport 설정을 만듭니다.  
...
먼저 가장 중요한 <kbd>passport.serializeUser</kbd>, <kbd>passport.deserializeUser</kbd>를 작성합니다.

req.login(user, ...) 가 실행되면, serializeUser가 실행됩니다.
즉, <u>로그인 과정을 할때만 실행</u>

deserializeUser는 serializeUser()가 done하거나 passport.session()이 실행되면 실행됩니다.  
...
router.post("/login", (req, res, next) => {
  ...
  passport.authenticate("local", (authError, user, info) => {
    ...
    if (!user) {
      // done()의 3번째 인자 { message: '비밀번호가 일치하지 않습니다.' }가 실행됩니다.
      return res.redirect(`/?loginError=${info.message}`);
    }

    //? done(null, exUser)가 처리된 경우
    return req.login(user, (loginError) => {
      //? loginError => 미들웨어는 passport/index.js의 passport.deserializeUser((id, done) => 가 done()이 되면 실행됩니다.
      // 만일 done(err) 가 됬다면,
      if (loginError) {
...
소셜 네트워킹의 등장으로 OAuth 공급자를 사용한 SSO(single sign on)이 널리 사용되는 인증 방법이 되었다. API를 호출하는 서비스는 액세스를 보호하기 위해 토큰 기반 자격 증명이 필요해졌습니다.

Passport는 각 응용 프로그램마다 고유의 인증 요구 사항이 있음을 인식하여 전략(Strategy)로 알려진 인증 메커니즘으로 개별 모듈로 패키지됩니다.
