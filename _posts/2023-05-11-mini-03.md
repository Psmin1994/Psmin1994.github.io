...
**_title_**에는 버튼안에 입력할 문자열을 넘겨줍니다.

**_onClickItem_**에는 **_useNavigate() Hook_**을 사용해 글 작성 페이지로 이동하는 함수를 넘겨줍니다.
...
BasicButton 컴포넌트에 props로 <kbd>posts</kbd>, <kbd>onClickItem</kbd>을 넘깁니다.

**_posts_**에는 글 정보를 담은 data를 넘겨주며, **_onClickItem_**에는 **_useNavigate() Hook_**을 사용해 글 상세 페이지로 이동하는 함수를 넘겨주겠습니다.
...
메인 페이지의 버튼과 같이 **_BasicButton 컴포넌트_**를 import해서 사용하겠습니다.

버튼의 위치를 조정하기위해 **_BtnWrapper_**로 감싸주고 `margin-left : auto`를 이용해 오른쪽에 붙여줍니다.

BasicButton 컴포넌트에 각각 props로 <kbd>title</kbd>, <kbd>onClickItem</kbd>을 넘깁니다.
...
**_rows_**는 화면에 보이는 값을 입력할 행의 수를 의미합니다.

**_value_**는 값 부분으로 useState로 설정한 변수를 의미합니다.

**_placeholder_**는 사용자가 값을 입력하기전에 입력할 내용을 알려주는 도움말을 의미합니다.

**_onClickItem_**는 **_useState() Hook_**을 사용해 값이 변하면 State를 업데이트해 재랜더링하는 함수를 의미합니다.
...
글 상세 페이지에서 메인 페이지로 돌아가기위한 버튼과 댓글 작성 완료 버튼을 구현해보겠습니다.
...
해당하는 글을 구분하기위해 parameter로 postId를 넘겨줬습니다.

이를 받기위해 `useParam Hook`을 사용합니다.

useParam Hook은 parameter 정보를 받아옵니다.

useParam을 import한 후 postId 정보를 받아온 후 해당 값을 key로 글 정보를 받아옵니다.
...
다음에는 express 서버와 연동해 데이터베이스에 글 정보에 대한 CRUD를 구현해보겠습니다.
