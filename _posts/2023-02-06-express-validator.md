...
간단한 예로 가입 절차를 진행하다가 **"이미 가입된 아이디입니다."**, **"비밀번호는 영문,숫자,특수문자가 혼합되어야 합니다"** 등 사용자는 개발자가 원하는 조건에 맞게 데이터를 입력하도록 유도합니다.
...
- **생성 메서드**
  ...
  - **body()**
    ...
    **check()**와 동일하지만 `req.body`만 확인합니다.

  - **cookies()**

    **check()**와 동일하지만 `req.cookies`만 확인합니다.

  - **headers()**

    **check()**와 동일하지만 `req.headers`만 확인합니다.

  - **params()**

    **check()**와 동일하지만 `req.params`만 확인합니다.

  - **query()**

    **check()**와 동일하지만 `req.query`만 확인합니다.
...
- **exists()**
  ...
  - `options.value`로 존재의 기준을 정합니다.

    - **undefined** : undefined 값을 기준으로 존재 확인
    - **null** : undefined, null 값을 기준으로 존재 확인
    - **falsy** : undefined, null, false, 잘못된 값(빈 문자열, 0 등) 값을 기준으로 존재 확인
...
- **isArray()**
  ...
  - `options.min`, `options.max` 값은 배열의 길이를 검사합니다.
...
- **isObject()**
  ...
  - `options.strict` 값을 `false`로 주면 순수 자바스크립트에서의 `typeof value ==='object'` 와 유사하게 동작합니다.
